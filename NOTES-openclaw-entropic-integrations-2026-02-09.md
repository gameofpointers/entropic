# Entropic + OpenClaw Integrations Context (2026-02-09)
...
- Entropic writes tool allowlist config so optional plugin tools are exposed to the model.
...
- Entropic integrations store path simplified to credentials dir.

Key changes

Entropic (repo: /home/alan/agent/Entropic, branch: skills-tasks-files)
- src-tauri/src/commands.rs
  - Ensure tools.alsoAllow includes "entropic-integrations" when applying agent settings.
  - Gateway health check now uses WS connect + health RPC (client id: openclaw-probe, token: entropic-local-gateway).
- openclaw-runtime/entrypoint.sh
  - Writes tools.alsoAllow ["entropic-integrations"] into openclaw.json at boot.
  - Keeps web.search.perplexity baseUrl when ENTROPIC_PROXY_MODE + ENTROPIC_PROXY_BASE_URL set.
...
OpenClaw (repo: /home/alan/agent/openclaw, branch: entropic)
- extensions/entropic-integrations/src/calendar-tools.ts
  - Renamed tools: calendar.list -> calendar_list, calendar.create -> calendar_create.
- extensions/entropic-integrations/src/gmail-tools.ts
  - Renamed tools: gmail.search -> gmail_search, gmail.get -> gmail_get, gmail.send -> gmail_send.
- extensions/entropic-integrations/src/store.ts
...
Recent commits
- Entropic: 6692c6d (Enable entropic integrations tool allowlist)
- OpenClaw: 307726bd4 (Rename Entropic integration tool names for OpenAI compatibility)
- OpenClaw: 22fa8c7bb (Update entropic integrations store path)
...
- Rebuild OpenClaw and runtime image, then restart container:
  ...
  - cd /home/alan/agent/Entropic && ./scripts/build-openclaw-runtime.sh
  - docker rm -f entropic-openclaw && relaunch Entropic
